...
//     </p>
//     <p class="text-muted">Release Date: ${movie.release_data}</p>
//     <p>
//     ${movie.overview}
//     </p>
//     <h5>Genres</h5>
...
//     <h5>Genres</h5>
//     <ul class="list-group">
//       ${tvShow.genres.map((genre) => `<li>${genre.name}</li>`)}
//     </ul>
...
//   </ul>
//   <h4>Production Companies</h4>
//   <div class="list-group">${tvShow.production_companies.map(
...
//   if (global.search.term !== "" && global.search.term !== null) {
//     const { results, total_pages, page, total_results } = await apiSearchData();
...
//   document.querySelector("#next").addEventListener("click", async () => {
//     global.search.page++;
//     const { results, total_pages } = await apiSearchData();
//     displaySearchResults(results);
//   });
...
//   switch (hash) {
//     case "home":
//       contentDiv.innerHTML = getPopularMovies();
//       displaySlider();
//       break;
//     case "about":
...
//   } else {
//     document.getElementById("content").innerHTML = "<h1>Default Page</h1>";
//   }
// }
...
async function displayMovieDetails() {
  ...
  <div class="details-top">
    ...
    <div>
      <h2>${movie.title}</h2>
      <p><i class="fas fa-star text-primary"></i> ${Math.floor(
        movie.vote_average
      )} / 10</p>
      <p class="text-muted">Release Date: ${movie.release_date}</p>
      <p>${movie.overview}</p>
...
async function displayShowDetails() {
  ...
  <div class="details-top">
    ...
    <div>
      ...
      <p>${tvShow.overview}</p>
      <h5>Genres</h5>
      <ul class="list-group">${tvShow.genres
        .map((genre) => `<li>${genre.name}</li>`)
        .join("")}</ul>
      <a href="#" target="_blank" class="btn">Visit Show Homepage</a>
    </div>
...
async function search() {
  ...
  if (global.search.term !== "" && global.search.term !== null) {
    ...
    showAlert("Please enter a search item", "alert-warning");
  }
}
...
function displayPagination() {
  ...
  document.querySelector("#next").addEventListener("click", async () => {
    global.search.page++;
    const { results } = await apiSearchData();
    displaySearchResults(results);
  });
